...
import {
  clearLoggedEvents,
  getLoggedEvents,
  expectValidSampleRand,
  expectDscInEnvelopeHeaders,
  getHttpServerTransactionsWithHeaders,
} from "./helpers";
...
test.describe("Tracing", () => {
  ...
  test("validates basic tracing functionality", async ({ page }) => {
    ...
    const errorTransactions = transactionEvents.filter(
      (event) => event.transaction?.includes("error") || event.transaction?.includes("GET")
    );
  ...
  test.describe("OpenTelemetry trace propagation", () => {
    test("validates trace IDs are properly generated for backend requests", async ({
      ...
      // Find the error transaction
      const errorTransaction = transactionEvents.find((event) =>
    ...
    test("validates child span data is preserved in distributed tracing", async ({
      ...
      const dataTransactions = transactionEvents.filter((event) =>
