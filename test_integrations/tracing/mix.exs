defmodule TracingTest.MixProject do
  ...
  defp setup_environment(_args) do
    ...
    System.cmd("npx", ["playwright", "install", "--only-shell", "--with-deps", "chromium"], into: IO.stream(:stdio, :line))
    ...
    # Setup svelte_mini dependencies
    svelte_mini_path = Path.join([__DIR__, "svelte_mini"])
    Mix.shell().info("Installing Svelte mini app dependencies...")
    System.cmd("npm", ["install"], cd: svelte_mini_path, into: IO.stream(:stdio, :line))
