defmodule Sentry.PlugTest do
  ...
  test "non-existent route exceptions are ignored" do
    ...
    assert ^exception =
      assert_raise(FunctionClauseError, "no function clause matching in Sentry.ExampleApp.do_match/4", fn ->
        conn(:get, "/not_found")
    ...
    scrubber =
      fn conn ->
        ...
        |> Enum.filter(fn {key, val} ->
          !(key in ~w(password passwd secret credit_card) || Regex.match?(~r/^(?:\d[ -]*?){13,16}$/, val))
        end)
        |> Enum.into(%{})
      end

    options = [
      body_scrubber: scrubber,
