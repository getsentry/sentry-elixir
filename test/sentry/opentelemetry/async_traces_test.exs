defmodule Sentry.Opentelemetry.AsyncTracesTest do
  ...
  describe "a grandchild span still running when both its parent and the root have ended" do
    test "is reported as a follow-up transaction of its own parent", %{bypass: bypass} do
      put_test_config(environment_name: "test", traces_sample_rate: 1.0)
      ref = setup_bypass_envelope_collector(bypass)
          ...
          Task.async(fn ->
            ...
            try do
              Tracer.with_span "async_parent" do
                ...
                {:ok, _pid} =
                  Task.start(fn ->
                    parent_token = :otel_ctx.attach(parent_ctx)

                    try do
                      Tracer.with_span "in_flight_grandchild" do
                        send(parent_pid, :grandchild_started)
                        send(test_pid, {:grandchild, self()})

                        receive do
                          :finish_grandchild -> :ok
                        end
                      end

                      send(test_pid, :grandchild_finished)
                    after
                      :otel_ctx.detach(parent_token)
                    end
                  end)

                receive do
                  :grandchild_started -> :ok
                after
                  1000 -> raise "grandchild span never started"
                end
              end
            after
      ...
      assert_receive {:grandchild, grandchild_pid}

      [root_tx] = collect_sentry_transactions(ref, 1)
      assert root_tx["transaction"] == "sync_root"

      root_span_descriptions = Enum.map(root_tx["spans"], & &1["description"])
      assert "async_parent" in root_span_descriptions
      refute "in_flight_grandchild" in root_span_descriptions

      parent_span = Enum.find(root_tx["spans"], &(&1["description"] == "async_parent"))

      send(grandchild_pid, :finish_grandchild)
      assert_receive :grandchild_finished, 1000

      assert [grandchild_tx] = collect_sentry_transactions(ref, 1),
             "no follow-up transaction was reported for the in-flight grandchild"

      assert grandchild_tx["transaction"] == "in_flight_grandchild"

      grandchild_trace = grandchild_tx["contexts"]["trace"]

      assert grandchild_trace["trace_id"] == root_tx["contexts"]["trace"]["trace_id"]
      assert grandchild_trace["parent_span_id"] == parent_span["span_id"]
      assert grandchild_trace["data"]["sentry.parent_span_already_sent"] == true
    end
  end
