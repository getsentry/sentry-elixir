defmodule Sentry.ClientTest do
  ...
  test "logs api errors" do
    ...
    try do
      ...
      e ->
        assert capture_log(fn -> Sentry.capture_exception(e) end) =~ ~r/400.*Creation of this event was denied due to rate limiting/
    end
  end
  ...
  test "errors when attempting to report invalid JSON" do
    ...
    capture_log(fn -> assert :error = Sentry.capture_message(unencodable_tuple) end)
  end
  ...
  test "calls anonymous before_send_event" do
    ...
    modify_env(:sentry, [
      ...
      before_send_event: fn e ->
        ...
        %{e |
          extra: Map.merge(e.extra, rest_metadata),
          user: Map.put(e.user, :id, user_id),
        }
      end,
    ...
    try do
      ...
      e ->
        assert capture_log(fn -> Sentry.capture_exception(e, result: :sync) end)
    end
  end
  ...
  test "calls anonymous after_send_event synchronously" do
    ...
    try do
      ...
      e ->
        assert capture_log(fn -> Sentry.capture_exception(e, result: :sync) end) =~ "AFTER_SEND_EVENT"
    end
  end
