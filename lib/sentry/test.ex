defmodule Sentry.Test do
  ...
  @config_registry :sentry_test_config_registry
  ...
  @doc false
  @spec fetch_test_config(atom()) :: {:ok, term()} | :error
  def fetch_test_config(key) do
    case :ets.whereis(@config_registry) do
      :undefined ->
        :error
      ...
      _ref ->
        ...
        Enum.find_value(pids, :error, fn pid ->
          case :ets.lookup(@config_registry, {pid, key}) do
            [{{^pid, ^key}, value}] -> {:ok, value}
            [] -> nil
          end
        end)
    end
  ...
  defp ensure_config_registry! do
    ensure_named_table!(@config_registry, [:named_table, :public, :set])
  end
  ...
  defp put_test_config(config) when is_list(config) do
    ...
    original_config =
      for {key, val} <- config do
        ...
        # Write to process dict (for current process) and persistent_term (fallback)
        Process.put({:sentry_test_config, renamed_key}, validated_val)
        :persistent_term.put({:sentry_config, renamed_key}, validated_val)

        # Write to config registry for $callers-based isolation in child processes
        if :ets.whereis(@config_registry) != :undefined do
          :ets.insert(@config_registry, {{test_pid, renamed_key}, validated_val})
        end
    ...
    ExUnit.Callbacks.on_exit(fn ->
      # Clean up config registry entries for this test
      if :ets.whereis(@config_registry) != :undefined do
        for {key, _, _} <- original_config do
          :ets.delete(@config_registry, {test_pid, key})
        end
      end
